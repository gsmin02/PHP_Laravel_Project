...
<form name="form1" method="post" action="{{ route('co.update', $row->id) }}{{ $tmp }}">
@csrf
@method('PATCH')
...
<tr>
	...
	<td width="80%" align="left">
		<div class="d-inline-flex">
			<input type="text" name="coname" size="20" maxlength="20" value="{{ $row -> coname }}" class="form-control form-control-sm">
		</div>
		<br>
...
<tr>
	...
	<td width="80%" align="left">
		<div class="d-inline-flex">
			<input type="text" name="cotel1" size="3" maxlength="3" value="{{ $cotel1 }}" class="form-control form-control-sm">-
			<input type="text" name="cotel2" size="4" maxlength="4" value="{{ $cotel2 }}" class="form-control form-control-sm">-
			<input type="text" name="cotel3" size="4" maxlength="4" value="{{ $cotel3 }}" class="form-control form-control-sm">
		</div>
		<br>
		...
		<div class="d-inline-flex">
			<select name="cokind" class="form-select form-select-sm">
				...
				@for($i = 0; $i < $n_cokind; $i++)
					@if($row->cokind == $a_cokind[$i])
						<option value="{{ $a_cokind[$i] }}" selected>{{ $a_cokind[$i] }}</option>
					@else
